...
class UserService:
    ...
    # async def _get_user_from_redis(self, user_id: int, role: str = "admin") -> bool:
    #     if (user := await redis.get_user_data(user_id)) is None:
    #         raise bad_request_error("admin is not in company")
    #     # if (user := await self.user_company_repo.get_by_user_id(user_id)):
    #     #     user_role = user.role
    #     return user
    ...
    async def get_user_by_link_code(self, id: int, link_code: int) -> Optional[UserItem]:
        ...
        if (user := await redis.get_user_data(id)) is None:
            raise bad_request_error("admin is not in company")
        if user["user_company_role"] != "admin":
